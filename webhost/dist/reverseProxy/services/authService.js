...
const cookieService_1 = require("./cookieService");
...
class AuthService {
    constructor(configuration) {
        ...
        this._proxyService = new proxyService_1.ProxyService(configuration.tokenEndpoint);
        this._cookieService = new cookieService_1.CookieService(configuration.cookieRootDomain, configuration.cookieEncryptionKey);
    }
    /*
     * Process an authorization code grant message
     */
    async authorizationCodeGrant(request, response) {
        // Proxy the request to the authorization server
        const clientId = this._validateAndGetClientId(request, false);
        logger_1.Logger.info(`Proxying Authorization Code Grant for client ${clientId}`);
        ...
        if (refreshToken) {
            ...
            const randomValue = crypto_1.randomBytes(32).toString('base64');
     ...
     */
    async refreshTokenGrant(request, response) {
        // Get the refresh token from the auth cookie
        const clientId = this._validateAndGetClientId(request, true);
        logger_1.Logger.info(`Proxying Refresh Token Grant for client ${clientId}`);
        const refreshToken = this._cookieService.readAuthCookie(clientId, request);
        // Send it to the Authorization Server
        const refreshTokenGrantData = await this._proxyService.sendRefreshTokenGrant(refreshToken, request, response);
    ...
    async expireRefreshToken(request, response) {
        const clientId = this._validateAndGetClientId(request, true);
        logger_1.Logger.info(`Expiring Refresh Token for client ${clientId}`);
    ...
    async clearCookies(request, response) {
        ...
        const clientId = this._validateAndGetClientId(request, true);
        logger_1.Logger.info(`Clearing Cookies for client ${clientId}`);
     ...
     */
    _validateAndGetClientId(request, requireCsrfCookie) {
    ...
    _validateOrigin(request) {
        if (request.headers && request.headers.origin) {
            if (request.headers.origin.toLowerCase() !== this._configuration.trustedWebOrigin.toLowerCase()) {
                throw errorHandler_1.ErrorHandler.fromSecurityVerificationError(`The origin header had an untrusted value of ${request.headers.origin}`);
            }
        }
    }
    /*
    ...
    _getClientId(request) {
        ...
        throw errorHandler_1.ErrorHandler.fromSecurityVerificationError('No client_id was found in the received form url encoded data');
    }
    /*
     ...
     */
    _validateCsrfCookie(clientId, request) {
        // Get the CSRF cookie
        ...
        if (!request.headers || !request.headers[this._requestHeaderFieldName]) {
            throw errorHandler_1.ErrorHandler.fromSecurityVerificationError('No CSRF request header field was supplied');
        }
        // Check that the values match
        if (cookieValue !== request.headers[this._requestHeaderFieldName]) {
            throw errorHandler_1.ErrorHandler.fromSecurityVerificationError('The CSRF request header does not match the CSRF cookie value');
        }
    }
}
